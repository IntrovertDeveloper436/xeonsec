    ...
    if (mustReinit) {
      ...
      const animate = () => {
        ...
        if (liquidEffect) {
          ...
          const tu = liqEffect.uniforms.get('uTime');
        ...
        if (composer) {
          ...
          composer.passes.forEach(p => {
            ...
            if (pass.effects) pass.effects.forEach(eff => { const tu = eff.uniforms?.get('uTime'); if (tu) tu.value = uniforms.uTime.value; });
          });
          composer.render();
      ...
      raf = requestAnimationFrame(animate);
      threeRef.current = { renderer, scene, camera, material, startTime, clickIx: 0, uniforms, resizeObserver: ro, raf, quad, timeOffset, composer, touch, liquidEffect };
    } else {
      const t = threeRef.current!;
      ...
      if (t.liquidEffect) {
        const le = t.liquidEffect as Effect & { uniforms: Map<string, THREE.Uniform> };
        const us = le.uniforms.get('uStrength'); if (us) us.value = liquidStrength;
        const uf = le.uniforms.get('uFreq');     if (uf) uf.value = liquidWobbleSpeed;
      }
